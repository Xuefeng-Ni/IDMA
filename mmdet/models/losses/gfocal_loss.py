...
@weighted_loss
def quality_focal_loss(pred, target, beta=2.0, alpha=0.6):
    ...
    #add alpha
##    loss = F.binary_cross_entropy_with_logits(
##        pred, zerolabel, reduction='none') * (1 - alpha)*2 * scale_factor.pow(beta)
    ...
    loss[pos, pos_label] = F.binary_cross_entropy_with_logits(
        pred[pos, pos_label], score[pos],
        reduction='none') * alpha*2 * scale_factor.abs().pow(beta)
    '''
