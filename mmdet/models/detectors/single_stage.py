...
##        self.att_loss = Heatmap()   #att_loss
    ...
    #eqfl
##    def _backward_fn_hook(self, module, grad_in, grad_out):
##        self.bbox_head.initial_loss_cls.collect_grad(grad_out[0])
    ...
    def forward_train(self,
        ...
        losses.update(self.bbox_head.forward_train(x, img_metas, gt_bboxes,gt_labels, gt_bboxes_ignore))
        '''
        '''
        # eqfl
        hook_head = "bbox_head"
        hook_cls_head = "tood_cls"
        assert hasattr(self, hook_head), f"hook_head: {hook_head} is not in current model"
        model_head = getattr(self, hook_head)
        assert hasattr(model_head, hook_cls_head), f"hook_cls_head: {hook_cls_head} is not in current head {hook_head}"
        model_cls_head = getattr(self.bbox_head, hook_cls_head)
        ...
        if isinstance(model_cls_head, nn.ModuleList):
            for sub_module in model_cls_head:
                sub_module.register_backward_hook(self._backward_fn_hook)
        else:
            model_cls_head.register_backward_hook(self._backward_fn_hook)
        '''
        return losses
