...
class ATSSHead(AnchorHead):
    ...
    def centerness_target(self, anchors, gts):
        ...
        left_right = torch.abs(left_right)
        top_bottom = torch.abs(top_bottom)
        centerness = torch.sqrt(
            (left_right.min(dim=-1)[0] / torch.clamp(left_right.max(dim=-1)[0], min=1e-8)) *
            (top_bottom.min(dim=-1)[0] / torch.clamp(top_bottom.max(dim=-1)[0], min=1e-8)))
    ...
    def _get_target_single(self,
        ...
        if len(neg_inds) > 0:
            label_weights[neg_inds] = 1/3   #1/3 per-class
        '''
        #rail seg
