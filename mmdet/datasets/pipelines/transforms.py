    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                ...
                #rail seg
                results['seg_img'] = np.zeros((560, 188, 3))
                seg_img, scale_factor = mmcv.imrescale(
                    results['seg_img'],
                    results['scale'],
                    return_scale=True,
                    backend=self.backend)
                results['seg_img'] = seg_img
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            for key in results.get('img_fields', ['img']):
                ...
                #rail seg
                results['seg_img'] = mmcv.imflip(
                    results['seg_img'], direction=results['flip_direction'])
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if self.size is not None:
                ...
                #rail seg
                padded_seg_img = mmcv.impad(
                    results['seg_img'], shape=self.size, pad_val=pad_val)
                results['seg_img'] = padded_seg_img
                ...
                #rail seg
                padded_seg_img = mmcv.impad_to_multiple(
                    results['seg_img'], self.size_divisor, pad_val=pad_val)
                results['seg_img'] = padded_seg_img
...
class RandomCrop:
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            #rail seg
            seg_img = results['seg_img'][crop_y1:crop_y2, crop_x1:crop_x2, ...]
            results['seg_img'] = seg_img
