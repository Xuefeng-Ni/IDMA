...
def compute_overlap(a, b, detected_annotations):
    ...
    if (interbox_idx[1].size != 0) & (a.shape[1] == 6):
        assigned_annotation = np.argmax(intersection / ua, axis=1)
        max_overlap = (intersection / ua)[0, assigned_annotation]
        for i in range(len(interbox_idx[1])):
            if not ((max_overlap >= 0.5) & (assigned_annotation not in detected_annotations)):
                if i != assigned_annotation:
                    interbox.append([max(a[0, 0], b[interbox_idx[1][i], 0]), max(a[0, 1], b[interbox_idx[1][i], 1]), min(a[0, 2], b[interbox_idx[1][i], 2]), min(a[0, 3], b[interbox_idx[1][i], 3]), area[interbox_idx[1][i]]])
...
def compute_overlap2(a, b, detected_annotations):
    ...
    if (interbox_idx[1].size != 0) & (a.shape[1] == 4):
        ...
        for i in range(len(interbox_idx[1])):
            if i != assigned_annotation:
                interbox_pr.append([max(a[0, 0], b[interbox_idx[1][i], 0]), max(a[0, 1], b[interbox_idx[1][i], 1]), min(a[0, 2], b[interbox_idx[1][i], 2]), min(a[0, 3], b[interbox_idx[1][i], 3]), (a[:, 2] - a[:, 0]) * (a[:, 3] - a[:, 1])])
                if max_overlap < 0.5:
    ...
    return intersection / ua, union_a, intersection / area, interbox, interbox_pr
...
def evaluate2(
    ...
    eval_mode = 'iou'   #'iou' 'iof' 'mm-iof' 'area-iof'
    ...
    for label in range(num_classes):  #generator.num_classes()
        ...
        for i in (all_annotations.keys() & all_detections.keys()):
            ...
            detections_eval = np.zeros((np.size(detections, 0), 6))
            annotations_eval = np.zeros((np.size(annotations, 0), 5))
            bbox_idx = 0

            if np.size(annotations, 0) == 0:
            ...
            for d in detections:
                ...
                if annotations.shape[0] == 0:
                    ...
                    detections_eval[bbox_idx][-1] = 0
                    bbox_idx += 1
                    continue

                overlaps, union_a, union_b, interbox = compute_overlap(np.expand_dims(d, axis=0), annotations, detected_annotations)
                ...
                cover_idx = np.where(union_b>=0.5, 1, 0)
                ...
                if eval_mode == 'iou':
                    ...
                    condition = (max_union_b >= iou_threshold and assigned_annotation not in detected_annotations)
                else:
                    condition = (max_union_b >= iou_threshold)
                if condition:
                    if eval_mode == 'area-iof':
                        false_positives = np.append(false_positives, 0-np.sum(union_b)+max_union_b)
                        true_positives  = np.append(true_positives, 1+np.sum(union_b)-max_union_b)
                    else:
                        false_positives = np.append(false_positives, 0)
                        true_positives  = np.append(true_positives, 1)
                        detections_eval[bbox_idx][-1] = 1
                        all_annotations[i][label][assigned_annotation, -1] = 1
                    ...
                    if eval_mode == 'area-iof':
                        false_positives = np.append(false_positives, 1-max_union_b)    #np.sum(union_b)
                        true_positives  = np.append(true_positives, np.sum(union_b))
                    else:
                        false_positives = np.append(false_positives, 1)
                        true_positives  = np.append(true_positives, 0)
                        detections_eval[bbox_idx][-1] = 0
        ...
        if label == 0:
            ...
            if eval_mode == 'mm-iof':
                print('{}: {}'.format('squats_recall', recall)) #single-multiple union>0.5
            else:
                if recall.size == 0:
