...
model = dict(
    ...
    bbox_head=dict(
        ...
        loss_iof=dict(type='IoULoss', loss_weight=1.0, mode='linear', iou_mode='iof'),
        loss_iog=dict(type='IoULoss', loss_weight=1.0, mode='linear', iou_mode='iog'),
...
img_norm_cfg = dict(
    mean=[0.742*255, 0.741*255, 0.731*255], std=[0.187*255, 0.191*255, 0.209*255], to_rgb=True)

optimizer = dict(_delete_=True, type='AdamW', lr=0.00005, betas=(0.9, 0.999), weight_decay=0.05,
                 paramwise_cfg=dict(custom_keys={'absolute_pos_embed': dict(decay_mult=0.),
                                                 'relative_position_bias_table': dict(decay_mult=0.),
                                                 'norm': dict(decay_mult=0.)}))
lr_config = dict(step=[27, 33]) #30, 40 27, 33
...
runner = dict(type='EpochBasedRunner', max_epochs=36)   #36 45
